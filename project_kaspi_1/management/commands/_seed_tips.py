...
class TipSeeder:
	...
	def getTotalResults(self,vid):
		try:
			r = requests.get(self.url.format(vid), params=self.params, timeout=5).json()
			if(r['meta']['code'] == 200):
	...
	def parseUrl(self, venue, page):
		...
		params['offset'] = page
		try:
			r = requests.get(self.url.format(venue.vid), params=self.params, timeout=5).json()
			if(r['meta']['code'] == 200):
	...
	def saveTip(self, item, venue):
		...
		try:
		 	tips_temp = getattr(venue,'tips')
			tips_temp.append(text)
			setattr(venue, 'tips', tips_temp)
			venue.save()
			self.success_tips += 1
		except:
		 	self.error_tips_decode += 1